...
const UserWithoutId = z.object({
  login: z.string({ required_error: "Login should be at least 5 characters long but not more than 20." }).min(5).max(20),
  password: z.string({ required_error: "Password should be at least 5 characters long but not more than 20." }).min(5).max(20),
